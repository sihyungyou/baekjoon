...
int main() {
    ...
    for (i = 1; i < N; i++) {
        if (i == N-1) {
            dp[i][0] = min(min(dp[i-1][1], dp[i-1][2]), min(dp[0][1], dp[0][2])) + arr[i][0];
            dp[i][1] = min(min(dp[i-1][0], dp[i-1][2]), min(dp[0][0], dp[0][2])) + arr[i][1];
            dp[i][2] = min(min(dp[i-1][1], dp[i-1][0]), min(dp[0][1], dp[0][0])) + arr[i][2];
        } else {
