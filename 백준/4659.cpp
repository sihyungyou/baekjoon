...
int main() {
    ...
    while(1) {
        ...
        c3 = true;
        for (i = 0; i < len; i++) {
            if (isvowel(str[i])) v = true;
            
            if (i < len - 1) {
                    if (str[i] == str[i+1]) {
                        if ( (str[i] == 'e' && str[i+1] == 'e') || (str[i] == 'o' && str[i+1] == 'o')) c2 = true;
                        else c2 = false;
                }
            }
            if (i < len - 2) {
                if ( (isvowel(str[i]) && isvowel(str[i+1]) && isvowel(str[i+2])) || (!isvowel(str[i]) && !isvowel(str[i+1]) && !isvowel(str[i+2])) ) c3 = false;
            }
        }
        
        if (v && c2 && c3) printf("<%s> is acceptable.\n", str);
